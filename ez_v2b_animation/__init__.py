...
普遍存在这几个问题：
  ...
  - 骨骼命名体系混乱：VRChat 标准命名、VRM 0.x、VRM 1.0、MMD 日文命名混用
  - 没有 IK，或 IK 手柄命名不可预测（LowerLeg.L.003 这种靠猜猜不出来）
  - 有 IK 但缺链末端旋转、缺控制器、缺分组，用起来别扭
  - 形态键体系不统一：vrc.blink_left / vrc.blink / vrc.Blink / まばたき 四种写法
  - FBX 导入后贴图路径失效，材质变紫
  - 换机器或传文件丢贴图

本插件针对这些场景做自动化处理。核心是**结构发现**：不写死任何骨骼名，
全部靠约束结构反查，所以换模型不用改代码。

实测支持（三套命名完全不同的 VRChat 模型全部通过）：
...
------------------------------------------------------------------
本插件由 B站 @高压郭炖大葱 制作并维护，主要面向 VRChat 模型优化。
使用、修改、二次分发请保留本署名。
------------------------------------------------------------------
"""
